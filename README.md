...
1.	Вероятности принадлежности введенного текста к некоторому классу, выбранного пользователем
2.	Вероятность принадлежности текста к имеющимся классам текущей библиотеки
Заметка: классификатор ползволяет судить о принадлежности текста к тому или иному классу, только в сравнении с другими классами текущей библиотеки.
...
После распаковки содержимого в желаемую директорию, перейти в папку doc можно сгененрировать автодокументацию используя команду в cmd
	doxygen DoxyFile
...
Находясь в папке src для проверки исходного кода, в командной строке, треубется выполнить команду: cppcheck --enable=all

## Запуск
Для запуска приложения необходимо собрать исходный код и запустить его в среде разработки. Полученный .exe файл требуется поместить в корневую папку с приложением (bayes_naive_classifier). Файл в котором содержится текст для проверки должен находиться в корневой папке.
Когда все это сделано, запускаем Slow_BC и выполняем дальнейшие инструкции появившиеся на экране.
