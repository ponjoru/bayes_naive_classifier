...
После распаковки содержимого в желаемую директорию, находясь в папке приложения, через командную строку сгенерировать файл опций, используя команду "doxygen -g 'DoxyFile'". 
Затем, находясь в папке с файлом опции, через командную строку сгенерировать автодокументацию, используя команду "doxygen 'DoxyFile'".

### Статистическая проверка кода

Для выполнения статической проверки кода необходимо установить утилиту cppcheck.
